...
export interface NormalizedWindow { source: "main" | "additional"; role: "primary" | "secondary"; label: string; usedPercent: number; windowSeconds: number; resetInSec: number; limitReached: boolean }
...
function normalizeWindow(label: string, window: unknown, limitReached: boolean, source: NormalizedWindow["source"], role: NormalizedWindow["role"]): NormalizedWindow | null { if (!isRecord(window)) return null; const usedPercent = toNumber(window.used_percent); const windowSeconds = toNumber(window.limit_window_seconds); if (usedPercent === undefined || windowSeconds === undefined || windowSeconds < 0) return null; return { source, role, label: labelForWindow(windowSeconds, label), usedPercent: Math.max(0, Math.min(100, usedPercent)), windowSeconds, resetInSec: resetInSeconds(window as UsageWindowPayload), limitReached } }
function normalizeRateLimit(label: string, rateLimit: unknown, source: NormalizedWindow["source"]): NormalizedWindow[] { if (!isRecord(rateLimit)) return []; const limitReached = rateLimit.limit_reached === true; return [normalizeWindow(label, rateLimit.primary_window, limitReached, source, "primary"), normalizeWindow(label, rateLimit.secondary_window, limitReached, source, "secondary")].filter((item): item is NormalizedWindow => Boolean(item)) }

export function normalizePayload(payload: UsagePayload): NormalizedPayload {
  const windows = normalizeRateLimit("Usage", payload.rate_limit, "main")
  if (Array.isArray(payload.additional_rate_limits)) for (const item of payload.additional_rate_limits) { if (!isRecord(item)) continue; const name = toStringValue(item.limit_name) ?? toStringValue(item.metered_feature) ?? "Extra"; for (const window of normalizeRateLimit(name, item.rate_limit, "additional")) windows.push({ ...window, label: window.label === "5h" || window.label === "Weekly" ? name : window.label }) }
